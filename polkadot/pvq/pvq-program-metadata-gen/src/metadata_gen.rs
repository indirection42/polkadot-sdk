...
/// Generates Rust source for a temporary crate that emits PVQ program metadata.
///
/// The returned token stream is meant to be written to a `main.rs` and compiled
/// as a standalone binary. The generated `main` writes two files into
/// `output_dir`:
///
/// - `{pkg_name}-metadata.bin` (SCALE encoded)
/// - `{pkg_name}-metadata.json` (JSON encoded)
///
/// `source` is expected to contain a module annotated with `#[program]` and
/// functions annotated with `#[program::entrypoint]` and/or
/// `#[program::extension_fn(extension_id = ..., fn_index = ...)]`.
pub fn metadata_gen_src(
	source: &str,
	pkg_name: &str,
	output_dir: &str,
				...
				if last_segment.ident == "extension_fn" {
					...
					attr.parse_nested_meta(|meta| {
						if meta.path.is_ident("extension_id") {
							...
							extension_id =
								Some(value.parse::<syn::LitInt>()?.base10_parse::<ExtensionId>()?);
						} else if meta.path.is_ident("fn_index") {
							let value = meta.value()?;
							fn_index =
								Some(value.parse::<syn::LitInt>()?.base10_parse::<FnIndex>()?);
						} else {
					...
					let extension_id = extension_id
						.ok_or_else(|| syn::Error::new(attr.span(), "Extension ID is required"))?;
					let fn_index = fn_index.ok_or_else(|| {
						syn::Error::new(attr.span(), "Function index is required")
					})?;
					let extension_fn_metadata =
						generate_extension_fn_metadata(item, extension_id, fn_index)?;
	...
	if entrypoints_metadata.is_empty() {
		return Err(syn::Error::new(proc_macro2::Span::call_site(), "No entrypoint function found"));
	}
...
fn metadata_defs() -> proc_macro2::TokenStream {
	quote! {
		type ExtensionId = u64;
		type FnIndex = u8;
		/// Serializable metadata describing a PVQ program's public surface.
		///
		/// This contains:
		/// - A SCALE type registry (`types`)
		/// - A list of extension functions (`extension_fns`)
		/// - A list of entrypoints (`entrypoints`)
		#[derive(Clone, PartialEq, Eq, Encode, Debug, Serialize)]
		pub struct Metadata {
			pub types: PortableRegistry,
			/// Extension functions keyed by `(extension_id, fn_index)`.
			///
			/// `extension_id` is encoded as a string to avoid loss of precision
			/// in JavaScript/TypeScript clients.
