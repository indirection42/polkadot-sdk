...
struct Args {
	...
	/// Directory where metadata artifacts will be written.
	///
	/// The generator writes:
	/// - `<CARGO_PKG_NAME>-metadata.bin` (SCALE)
	/// - `<CARGO_PKG_NAME>-metadata.json` (JSON)
	#[arg(short, long)]
	output_dir: PathBuf,

	/// Optional path to a `Cargo.toml` used for the temporary generator crate.
	///
	/// This should be a manifest that contains all dependencies required to
	/// compile the PVQ program module after it is transplanted into the temp
	/// crate. If omitted, a minimal default manifest is used.
	#[arg(short, long)]
...
fn main() {
	...
	let (manifest_content, optional_features) = if let Some(ref manifest_path) = args.manifest_path
	{
		...
		let content = std::fs::read_to_string(manifest_path).unwrap_or_else(|_| {
			panic!("Failed to read manifest file: {}", manifest_path.display())
		});
	...
	let metadata_gen_src = pvq_program_metadata_gen::metadata_gen_src(
		&source,
		&pkg_name,
	...
	fs::create_dir_all(temp_crate_path.join("src"))
		.expect("Failed to create `temp_crate/src directory");
