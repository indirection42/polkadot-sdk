...
impl Def {
	pub fn try_from(mut item: syn::ItemMod) -> syn::Result<Self> {
					...
					if last_segment.ident == "extension_fn" {
						...
						attr.parse_nested_meta(|meta| {
							if meta.path.is_ident("extension_id") {
								...
								extension_id =
									Some(value.parse::<syn::LitInt>()?.base10_parse::<u64>()?);
							} else if meta.path.is_ident("fn_index") {
								let value = meta.value()?;
								fn_index =
									Some(value.parse::<syn::LitInt>()?.base10_parse::<u32>()?);
							} else {
						...
						let extension_fn =
							ExtensionFn::try_from(attr.span(), item, extension_id, fn_index)?;
		...
		if entrypoints.is_empty() {
			return Err(syn::Error::new(mod_span, "At least one entrypoint function is required"));
		}

		// Put remaining items back
		items.extend(remaining_items);

		let def = Def { item, extension_fns, entrypoints, parity_scale_codec, polkavm_derive };

		Ok(def)
	}
}

/// Additional keywords used by the parser.
mod keyword {
