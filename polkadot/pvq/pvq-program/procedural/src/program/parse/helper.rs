...
/// Remove and return the first `#[program::...]` attribute from the given item (if present).
pub(crate) fn take_first_program_attr(
	...
	let Some(index) = attrs.iter().position(|attr| {
		attr.path().segments.first().is_some_and(|segment| segment.ident == "program")
	}) else {
		return Ok(None);
	};
